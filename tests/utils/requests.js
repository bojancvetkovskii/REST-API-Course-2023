...
export async function request(context, method, path, body = undefined, auth = true, asserts = { statusCode: 200 }, host = undefined, customHeaders = undefined) {
    ...
    const headers = customHeaders ? customHeaders : {
        ...
        'Authorization': `Bearer ${config[global.env].token}`
    }
    ...
    switch (method) {
        case 'GET':
            ...
            // getLastElementAsResponse is a flag which shows that the last element of the array should be taken as a responseBody
            if (asserts.getLastElementAsResponse && responseBody.length > 0) {
                responseBody = responseBody[responseBody.length-1]
            }

            // find the element from the response list which contains the id - findObjectWithId
            if (asserts.findObjectWithId) {
                for (let index in responseBody) {

                    if (responseBody[index].id === asserts.findObjectWithId) {
                        responseBody = responseBody[index]
                        break;
                    }
                }
            }
